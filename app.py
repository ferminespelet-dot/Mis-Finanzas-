...
<style>
    ...
    @media print {
        header, .stTabs, .stSidebar, button {
...
if df_tx.empty:
    df_tx = pd.DataFrame(columns=["tipo", "monto", "descripcion", "categoria", "timestamp"])

df_tx["timestamp_dt"] = pd.to_datetime(df_tx["timestamp"], errors='coerce')
mask_curr = df_tx["timestamp_dt"] >= current_cycle_start

df_curr = df_tx[mask_curr]

def obtener_sumas(df):
    if df.empty: return 0, 0, 0, 0, 0
    i = df[df["tipo"] == "ingreso"]["monto"].sum()
    g = df[df["tipo"] == "gasto"]["monto"].sum()
    a = df[df["tipo"] == "ahorro"]["monto"].sum()
    d = df[df["tipo"] == "desahorro"]["monto"].sum()
    r = df[df["tipo"] == "rollover"]["monto"].sum()
    return i, g, a, d, r
...
def formatear_tarjeta_movimiento(tx):
    tipo = tx["tipo"]
    if tipo in ["ingreso", "rollover", "desahorro"]:
        color = "#10B981"
        signo = "+"
    else:
        color = "#EF4444"
...
if not st.session_state.get("mostrar_reporte", False):
    ...
    with tab_registro:
        ...
        <div style='background-color: rgba(16, 185, 129, 0.1); border: 2px solid rgba(16, 185, 129, 0.3); padding: 20px; border-radius: 15px; text-align: center; margin-bottom: 25px;'>
            ...
            <h1 style='margin: 0; font-size: 52px; font-weight: 900; color: #10B981;'>{simbolo_html}{saldo_ciclo_actual:,.0f}</h1>
        </div>
        """.replace(",", "."), unsafe_allow_html=True)
        
        st.markdown("### 🎙️ Nuevo Registro")
        user_input = st.text_area("Registro", placeholder="Ej: Gaste 2000 en bondi... o Guardé 5000 en Ahorro General", height=100, label_visibility="collapsed")
        ...
        if procesar and user_input.strip():
            with st.spinner("La IA está procesando..."):
                try:
                    ...
                    REGLAS IMPORTANTES:
                    1. Si el usuario dice EXPLÍCITAMENTE "mitad y mitad" o "mitad cada uno", agrega `"split": true`.
                    2. Si el usuario dice que "ahorró", "guardó en la alcancía" o "separó" plata, el TIPO debe ser "ahorro" y en "categoria" pon el nombre del bolsillo (por defecto "Ahorro General").
                    ...
                    for tx in data.get("movimientos", []):
                        ...
                        if tx["tipo"] == "ahorro":
                            ...
                            if not target_b:
                                target_b = next((b for b in st.session_state.bolsillos if b["id"] == "b_default"), st.session_state.bolsillos[0])
                            
                            target_b["monto"] += final_monto
                            save_table(f"Bolsillos_{sufijo}", ["id", "nombre", "ubicacion", "monto", "wishlist_id"], st.session_state.bolsillos)
                            
                            tx_propia = {"timestamp": ts, "tipo": "ahorro", "monto": final_monto, "descripcion": tx.get("descripcion", "Ahorro"), "categoria": "Ahorro en Alcancía"}
                            ...
                            if is_split:
                                ...
                                st.success(f"¡Mitad anotada para vos y la otra mitad enviada a {otro_sufijo}!")
    ...
    with tab_cerebro:
                    ...
                    try:
                        ...
                        if "ey, cerebro" in raw_thought.lower() or "ey cerebro" in raw_thought.lower():
                            ...
                            thread_obj = {
                                ...
                                "creado": ts,
                                "mensajes": [{"autor": "usuario", "texto": raw_thought}, {"autor": "assistant", "texto": respuesta_texto}]
                            }
        ...
        if st.session_state.thoughts:
            hilos_ordenados = sorted([t for t in st.session_state.thoughts if isinstance(t, dict)], key=lambda x: str(x.get("id", "")), reverse=True)
                ...
                with st.expander(f"{t['titulo']}  (Creado: {t['creado'][:10]})"):
                    ...
                    for msg in t["mensajes"]:
                        if msg["autor"] == "usuario":
                            with st.chat_message("user", avatar="👤"): st.write(msg["texto"])
                        else:
                            with st.chat_message("assistant", avatar="🧠"): st.write(msg["texto"])
                    
                    with st.form(f"chat_form_{t['id']}", clear_on_submit=True):
                        reply = st.text_area("Nueva nota...", placeholder="Anota algo o escribe 'Ey, Cerebro...'", key=f"input_{t['id']}", height=80)
    ...
    with tab_wishlist:
        ...
        if st.button("Buscar y Comparar Precios", type="primary", use_container_width=True):
            if not api_key:
                ...
                with st.spinner("Analizando mercado y armando tarjetas..."):
                    try:
                        client = genai.Client(api_key=api_key)
                        prompt = f"""
                        ...
                        {{
                            "analisis": "Texto breve de 2 líneas comparando opciones.",
                            "opciones": [
                                {{
    ...
    with tab_balance:
        ...
        if not df_curr.empty and not df_curr[df_curr["tipo"] == "gasto"].empty:
            df_g = df_curr[df_curr["tipo"] == "gasto"].groupby("categoria")["monto"].sum().reset_index().sort_values(by="monto", ascending=False)
    ...
    with tab_historial:
        ...
        if not df_tx.empty:
            ...
            df_show = df_tx.copy().sort_values(by="timestamp", ascending=False)
            ...
            def get_friendly_cat(row):
                if row["tipo"] == "rollover": return "Traspaso de mes"
                elif row["tipo"] == "ahorro": return "Ahorro"
                elif row["tipo"] == "desahorro": return "Desahorro"
                else: return str(row["categoria"]).capitalize()
                
            df_display["Categoría"] = df_display.apply(get_friendly_cat, axis=1)
            df_display["Descripción"] = df_display["descripcion"].str.capitalize()
            
            def format_monto(row):
                if row["tipo"] in ["ingreso", "desahorro", "rollover"]: 
                    return f"+ {simbolo_moneda}{row['monto']:,.0f}".replace(",", ".")
                else: 
                    return f"- {simbolo_moneda}{row['monto']:,.0f}".replace(",", ".")
    ...
    with tab_ciclos:
        ...
        st.markdown(f"**Disponible de meses anteriores:** <span style='color:#10B981; font-weight:bold;'>{simbolo_html}{sobrante_pasado:,.0f}</span>".replace(",", "."), unsafe_allow_html=True)
        st.caption("*(Tu sueldo del ciclo actual está protegido y no se puede guardar en el pasado. Para usar tu sobrante histórico, transfierelo al ciclo actual).*")

        if sobrante_pasado > 0:
            opc_bolsillos_solo = {}
            ...
            opciones_destino = {"rollover": "⏩ Mover al saldo de ESTE mes (Para gastar)"}
            opciones_destino.update(opc_bolsillos_solo)

            leftover = st.number_input("Monto a asignar:", min_value=0.0, max_value=float(sobrante_pasado), value=float(sobrante_pasado), step=1000.0)
            destino_id = st.selectbox("¿Qué querés hacer con el sobrante?", options=list(opciones_destino.keys()), format_func=lambda x: opciones_destino[x])
            
            if st.button("📥 Asignar Dinero", use_container_width=True) and leftover > 0:
                ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                if destino_id == "rollover":
                    tx_propia = {"timestamp": ts, "tipo": "rollover", "monto": leftover, "descripcion": "Sobrante transferido al mes actual", "categoria": "Traspaso"}
                    st.session_state.transactions.append(tx_propia)
                    st.success(f"¡{simbolo_moneda}{leftover} sumados a tu Saldo del Mes actual!")
                else:
                    for b in st.session_state.bolsillos:
                        if b["id"] == destino_id:
                            b["monto"] += leftover
                    tx_propia = {"timestamp": ts, "tipo": "ahorro", "monto": leftover, "descripcion": "Ahorro desde sobrante", "categoria": "Ahorro en Alcancía"}
                    st.session_state.transactions.append(tx_propia)
                    save_table(f"Bolsillos_{sufijo}", ["id", "nombre", "ubicacion", "monto", "wishlist_id"], st.session_state.bolsillos)
                    st.success(f"¡Guardado en {opciones_destino[destino_id]}!")
                
                save_table(f"Gastos_{sufijo}", ["timestamp", "tipo", "monto", "descripcion", "categoria"], st.session_state.transactions)
                st.rerun()
        else:
            st.info("No tenés plata libre de meses anteriores para asignar.")

        st.divider()
        st.subheader("💰 Mis Bolsillos (Alcancía)")
        st.metric("Total Ahorrado", f"{simbolo_moneda}{total_ahorrado_en_bolsillos:,.0f}".replace(",", "."))
        ...
        for b in st.session_state.bolsillos:
            with st.container(border=True):
                ...
                with col_mov:
                    ...
                    opc_mover = {"saldo_principal": "⏪ Devolver al Saldo Principal"}
                    opc_mover.update({bk["id"]: bk["nombre"] for bk in st.session_state.bolsillos if bk["id"] != b["id"]})
                    
                    dest = st.selectbox("Destino", [x for x in opc_mover.keys()], format_func=lambda x: opc_mover[x], key=f"dest_{b['id']}")
                    if st.button("Transferir", key=f"btn_mov_{b['id']}") and mover > 0:
                        b["monto"] -= mover
                        if dest == "saldo_principal":
                            ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                            st.session_state.transactions.append({"timestamp": ts, "tipo": "desahorro", "monto": mover, "descripcion": f"Desahorro desde {b['nombre']}", "categoria": "Desahorro"})
                            save_table(f"Gastos_{sufijo}", ["timestamp", "tipo", "monto", "descripcion", "categoria"], st.session_state.transactions)
                        else:
    ...
    with tab_ajustes:
        ...
        st.info("Generá una vista limpia para imprimir o guardar como PDF.")
        if st.button("Ver Reporte Listo para Imprimir", use_container_width=True):
        ...
        with st.expander("⚠️ Zona de Peligro"):
            ...
            if st.button("🗑️ Borrar todos mis datos y empezar de cero", type="primary", use_container_width=True):
                ...
                st.success("¡Datos borrados con éxito! La app está como nueva.")
                st.rerun()
...
# ==============================================================
# VISTA EXCLUSIVA DE REPORTE (HTML LIMPIO PARA PDF)
# ==============================================================
else:
    st.markdown("<div class='report-container'>", unsafe_allow_html=True)
    st.markdown(f"<h1 style='text-align: center; color: black;'>Reporte Financiero - {usuario}</h1>", unsafe_allow_html=True)
    st.markdown(f"<p style='text-align: center; color: gray;'>Generado el {datetime.datetime.now().strftime('%d/%m/%Y')}</p>", unsafe_allow_html=True)
    st.divider()
    
    col_a, col_b, col_c = st.columns(3)
    col_a.metric("Ingresos del Ciclo", f"{simbolo_moneda}{i_c:,.0f}".replace(",", "."))
    col_b.metric("Gastos del Ciclo", f"{simbolo_moneda}{g_c:,.0f}".replace(",", "."))
    col_c.metric("Total Ahorrado", f"{simbolo_moneda}{total_ahorrado_en_bolsillos:,.0f}".replace(",", "."))
    
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("### 📊 Gráfico de Gastos del Ciclo")
    if not df_curr.empty and not df_curr[df_curr["tipo"] == "gasto"].empty:
        df_g = df_curr[df_curr["tipo"] == "gasto"].groupby("categoria")["monto"].sum().reset_index()
        fig = px.pie(df_g, values="monto", names="categoria", hole=0.45)
        fig.update_traces(textinfo='label+percent', textfont_size=14)
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.write("No hay gastos registrados en este ciclo.")

    st.markdown("### 📜 Detalle de Movimientos del Ciclo")
    if not df_curr.empty:
        df_rep = df_curr.copy().sort_values(by="timestamp", ascending=False)
        df_rep["Fecha"] = pd.to_datetime(df_rep["timestamp"]).dt.strftime("%d/%m")
        def format_rep(row):
            signo = "+" if row["tipo"] in ["ingreso", "rollover", "desahorro"] else "-"
            return f"{signo} {simbolo_moneda}{row['monto']:,.0f}".replace(",", ".")
        df_rep["Monto"] = df_rep.apply(format_rep, axis=1)
        st.table(df_rep[["Fecha", "categoria", "descripcion", "Monto"]].rename(columns={"categoria": "Categoría", "descripcion": "Descripción"}))
    
    st.markdown("</div>", unsafe_allow_html=True)
    st.info("🖨️ Presiona **Ctrl + P** (o Cmd + P en Mac) en tu navegador y selecciona 'Guardar como PDF' o 'Imprimir'.")
    
    if st.button("⬅️ Volver a la Aplicación", type="primary", use_container_width=True):
